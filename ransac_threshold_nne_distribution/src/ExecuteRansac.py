...
def run(inputfilepath:str,outputfolder:str,max_lines_to_find:int,threshold_factor:float):
    ...
    file_result=os.path.join(outputfolder,("result-%s-tfac-%.2f.png") % (filename_noextension,round(threshold_factor,2)))
...
def execute_ransac_on_files(csvfile:str,threshold_factors:List[float]):
    ...
    for input_datarow in input_datarows:        
        for threshold_factor in threshold_factors:
            ...
            run(inputfilepath=absolute_input_file,outputfolder=new_directory_for_results,max_lines_to_find=input_datarow.line_count, threshold_factor=threshold_factor)
            result=OutputRow()
            result.imagefile=input_datarow.imagefile
            ransac_results.append(result)
            #you where hee, write to a central dictionary wrapper #keep appending and then address the temp_output_folder issue
            #writ the final results to a CSV in the output folder
            pass
...
def main():
    ###
    #remove this
    execute_ransac_on_files("C:\\Users\saurabhd\\MyTrials\\MyGithubPage\\ransac_threshold_nne_distribution\\src\\out\\small_dataset_1line\\input_images.csv", threshold_factors=[0.25,0.5, 0.1])
    return
    ###
