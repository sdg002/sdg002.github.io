...
class Util(object):
    ...
    def superimpose_straight_line_between_2_points(input_image:np.ndarray,start_x:float, start_y:float, end_x:float, end_y:float, max_distance:float):
        ...
        distance_start_end=Util.euclidean_distance(x1=start_x, y1=start_y, x2=end_x, y2=end_y)
        ...
        while True:
            ...
            consecutive_distance=Util.euclidean_distance(x1=last_point[0], y1=last_point[1], x2=next_point[0], y2=next_point[1])
        ...
        for point in results:
            ...
            if (y<0 or y >= height):
                continue
            input_image[y][x][0]=Util.BLACK_COLOR
        return input_image
        pass
    ...
    def superimpose_all_ransac_lines(image:np.ndarray,ransac_lines:List[RansacLineInfo]):
        ...
        for line_index in range(0,len(ransac_lines)):
            ...
            x_min=min(x_values)
            x_max=max(x_values)
            plottable_points=Util.generate_plottable_points_along_line(ransac_lineinfo.scikit_model, xmin=x_min,xmax=x_max, ymin=y_min,ymax=y_max)
