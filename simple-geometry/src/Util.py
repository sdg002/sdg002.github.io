...
class Util(object):
    ...
    #
    @classmethod
    def create_points_from_numpyimage(cls, arr_image:np.ndarray):
        pass
        lst=list()
    ...
    #
    @classmethod
    def add_points_to_monoimage(cls,arr_image_input:np.ndarray, points,use_black:bool):
        width=arr_image_input.shape[1]
        height=arr_image_input.shape[0]
        for p in points:
            x:int=int(round(p.X))
    ...
    #
    @classmethod
    def plot_line_2darray(cls,np_array,x_start,y_start,x_end,y_end,num_points):
    ...
    #
    @classmethod
    def get_terminal_points_from_coliner_points(cls,points:List[Point]):
        if (len(points)<2):
